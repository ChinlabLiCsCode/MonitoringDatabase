...
class _Thorlabs_TSP01B_libusb:
    ...
    def __init__(self, serial_number: str) -> None:
        ...
        Raises:
            Exception: When the specified instrument is not found.
        ...
        if self._dev is None:
            pass
            raise Exception("Instrument with serial number {} not found (check device permission)"
        ...
        if hid_interface is None:
            raise Exception("Instrument does not have a HID interface")
        ...
        if endpoint_in is None:
            raise Exception("Instrument does not have Interrupt IN endpoint")
        if endpoint_out is None:
            raise Exception("Instrument does not have Interrupt OUT endpoint")
            pass
...
class TSP01():
    ...
    def _check_is_open(self) -> None:
        ...
        if not self._is_open:
            _logger.error("Interaction with closed instrument %s not allowed", self._serial_number)
            raise Exception(
                "Operation not allowed on closed instrument {}".format(self._serial_number))

    def _check_is_closed(self) -> None:
        """Verify that the instrument is closed, otherwise raise an exception."""
        if self._is_open:
            _logger.error("Interaction with open instrument %s not allowed", self._serial_number)
            raise Exception(
                "Operation not allowed on open instrument {}".format(self._serial_number))
    ...
    def open(self) -> None:
        _logger.info("[%s] Opening connection to instrument", self._serial_number)
    ...
    def close(self) -> None:
        _logger.info("[%s] Closing connection to instrument", self._serial_number)
        #self._check_is_open()
    ...
    def _query_device_info(self, info_type: int) -> str:
        ...
        if (reply_par != 0x04) or (len(reply_data) < 1) or (reply_data[0] != info_type):
            raise Exception("Unexpected reply to device info query ({:02x}, {})"
    ...
    def get_idn(self) -> str:
        """Read instrument type and version and return string."""
        self._check_is_open()
    ...
    def reset(self):
        ...
        if (reply_par != 0x04) or (reply_data != bytes([0x00])):
            raise Exception("Unexpected reply to reset command ({:02x}, {})"
                                          .format(reply_par, self._bytes_to_hex(reply_data)))
    ...
    def _query_measurement(self, channel: int, ext_channel: int = 0) -> float:
        ...
        Raises:
            Exception: If the selected external sensor is not connected.
        """
        ...
        if (reply_par != 0x07) or (len(reply_data) != 5) or (reply_data[0] != channel):
            raise Exception("Unexpected reply to measurement query ({:02x}, {})"
                                          .format(reply_par, self._bytes_to_hex(reply_data)))
    ...
    def getHumidity(self) -> float:
        ...
        #get the humidity and return it
        self.humidity = self._query_measurement(self._CHANNEL_HUMIDITY)

        return self.humidity

    def getMainTemp(self) -> float:
        ...
        #get the main temperature and return it
        self.mainTemp = self._query_measurement(self._CHANNEL_TEMPERATURE)
        #print(self.mainTemp)

        return self.mainTemp

    def getChTemp(self, ext_channel: int) -> float:
        ...
        #get the external temperature for a particular channel and return it
        self.chTemp = self._query_measurement(self._CHANNEL_EXTERNAL, ext_channel-1)
        #print(self.chTemp)
